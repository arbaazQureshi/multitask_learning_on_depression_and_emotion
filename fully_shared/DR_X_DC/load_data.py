...
def load_CMU_MOSEI_training_data():
	
	location = '/home/syedcs15/depression_estimation/preprocessed_data/training_data/CMU_MOSEI_transcript/X_train.npy'

	#train_set_ID_list = os.listdir(location)
	#train_set_ID_list.remove('labels')
	#print(train_set_ID_list)
	
	X = np.load(location)
	#X = []

	#for ID in train_set_ID_list:
		#if(os.path.isfile(location+ID)):
		#X.append(np.load(location + ID))

	#print(train_set_ID_list[1].split('.')[0])

	Y = np.load('/home/syedcs15/depression_estimation/preprocessed_data/training_data/CMU_MOSEI_transcript/Y_train.npy')

	#for ID in train_set_ID_list:
	#	Y.append(np.load(location + 'labels/' + ID.split('.')[0]+'_labels.npy'))
	...
	print("CMU MOSEI training data is loaded.")

	return (X, Y)


def load_CMU_MOSEI_development_data():
	
	location = '/home/syedcs15/depression_estimation/preprocessed_data/development_data/CMU_MOSEI_transcript/X_dev.npy'
	...
	Y = np.load('/home/syedcs15/depression_estimation/preprocessed_data/development_data/CMU_MOSEI_transcript/Y_dev.npy')
	...
	print("CMU MOSEI development data is loaded.")
