...
def loss_DW(y_true, y_pred):
	MSE = K.mean(K.square(y_pred[0] - y_true[0]))
	adversarial_loss = categorical_crossentropy(y_true[1], y_pred[1])
	return MSE + adversarial_loss


def metric_DW_1(y_true, y_pred):
	MSE = K.mean(K.square(y_pred[0] - y_true[0]))
	return MSE
'''
...
loss_funcs_DW = {'DW_output_layer' : 'mse', 'shared_discriminator_output_layer' : 'categorical_crossentropy'}
loss_weights_DW = {'DW_output_layer' : 0.78, 'shared_discriminator_output_layer' : 0.22}
metrics_DW = {'DW_output_layer' : ['mse', 'mae']}


loss_funcs_CM = {'CM_output_layer' : mean_squared_error, 'shared_discriminator_output_layer' : 'categorical_crossentropy'}
loss_weights_CM = {'CM_output_layer' : 0.78, 'shared_discriminator_output_layer' : 0.22}
...
while(True):
	...
	for epoch in range(25):
		...
		DW_model.fit(X_train_DW, [Y_train_DW, Y_train_DW_task], batch_size = X_train_DW.shape[0])
		DW_model.save_weights('DW_model_weights.h5')
		loss_dev_DW = DW_model.evaluate(X_dev_DW, [Y_dev_DW, Y_dev_DW_task], batch_size = X_dev_DW.shape[0])
		
		CM_model.load_weights('DW_model_weights.h5', by_name = True)
		loss_dev_CM = CM_model.evaluate(X_dev_CM, [Y_dev_CM, Y_dev_CM_task])
	...
	for epoch in range(25):
		...
		CM_model.fit(X_train_CM, [Y_train_CM, Y_train_CM_task], batch_size = X_train_CM.shape[0])
		CM_model.save_weights('CM_model_weights.h5')
		loss_dev_CM = CM_model.evaluate(X_dev_CM, [Y_dev_CM, Y_dev_CM_task])

		DW_model.load_weights('CM_model_weights.h5', by_name = True)
		loss_dev_DW = DW_model.evaluate(X_dev_DW, [Y_dev_DW, Y_dev_DW_task])
