...
def load_DAIC_WOZ_model(location=None):
	...
	Y1 = CuDNNLSTM(130, name = 'private_DW_lstm_layer', return_sequences = True)(X)
	...
	H = Dense(50, activation = 'tanh', name = 'DW_attention_hidden_layer_1')(H)
	...
	YR = Dense(45, activation = 'relu', name = 'DWR_hidden_layer_1')(F)
	YR = Dropout(rate = 0.25)(YR)
	
	YR = Dense(1, activation = None, name = 'DWR_output_layer')(YR)


	YC = Dense(45, activation = 'relu', name = 'DWC_hidden_layer_1')(F)
	YC = Dropout(rate = 0.25)(YC)

	YC = Dense(5, activation = 'softmax', name = 'DWC_output_layer')(YC)
	...
	model = Model(inputs = X, outputs = [YR, YC, Y_discriminator_output, Y_diff])
