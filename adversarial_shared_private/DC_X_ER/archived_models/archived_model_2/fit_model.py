...
loss_funcs_DW = {'DWC_output_layer' : 'categorical_crossentropy', 'shared_discriminator_output_layer' : 'categorical_crossentropy', 'DW_L_diff_layer' : L_diff}
loss_weights_DW = {'DWC_output_layer' : 1.09, 'shared_discriminator_output_layer' : 0.25, 'DW_L_diff_layer' : 0.08}
metrics_DW = {'DWC_output_layer' : ['accuracy']}
...
while(total_epoch_count < 400):
	...
	for epoch in range(epochs_on_a_task):
		...
		DW_model.fit(X_train_DW, [Y_train_DW_class, Y_train_DW_task, np.ones((X_train_DW.shape[0],))], batch_size = X_train_DW.shape[0])
		DW_model.save_weights('DW_model_weights.h5')
		loss_dev_DW = DW_model.evaluate(X_dev_DW, [Y_dev_DW_class, Y_dev_DW_task, np.ones((X_dev_DW.shape[0],))], batch_size = X_dev_DW.shape[0])
		...
		if(loss_dev_DW[4] > max_DC_ACC):
			print("DC_ACC "*200)

			max_DC_ACC = loss_dev_DW[4]
			...
			with open('DC_ACC_current_best.txt', 'w') as f:
				f.write('DC_accuracy:\t'+str(loss_dev_DW[4]))
				f.write('\n')
				f.write('DC_crossentropy:\t'+str(loss_dev_DW[1]))
				f.write('\n')
				...
				f.write('\n')
				f.write('DW_L_diff:\t'+str(loss_dev_DW[3]))
				f.write('\n')
		...
		if(loss_dev_DW[1] < min_DC_crossentropy):
			print("DC_crossentropy "*20)

			min_DC_crossentropy = loss_dev_DW[1]
	...
	for epoch in range(epochs_on_a_task):
		...
		DW_model.load_weights('ER_model_weights.h5', by_name = True)
		loss_dev_DW = DW_model.evaluate(X_dev_DW, [Y_dev_DW_class, Y_dev_DW_task, np.ones((X_dev_DW.shape[0],))])
